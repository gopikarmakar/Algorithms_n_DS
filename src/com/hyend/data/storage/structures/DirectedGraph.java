...
public class DirectedGraph {
	...
	public DirectedGraph reverse() {		
		DirectedGraph diGraph = new DirectedGraph(this.vertices());
		for(int v = 0; v < this.vertices(); v++) {
	...
	class DirectedTraversals {		
		...
		public DirectedTraversals() {
			edgeTo = new int[DirectedGraph.this.vertices()];
			marked = new boolean[DirectedGraph.this.vertices()];
		}
	...
	class DirectedCycle {		
		...
		public DirectedCycle() {
			edgeTo = new int[DirectedGraph.this.vertices()];
			marked = new boolean[DirectedGraph.this.vertices()];
			onStack = new boolean[DirectedGraph.this.vertices()];
			for(int v = 0; v < DirectedGraph.this.vertices(); v++) {
				if(!marked[v]) dfs(v);
			}
		}
	...
	class StrongConnections {		
		...
		public StrongConnections() {
			id = new int[DirectedGraph.this.vertices()];
			marked = new boolean[DirectedGraph.this.vertices()];
			DepthFirstOrder order = new DepthFirstOrder(DirectedGraph.this.reverse());
	...
	class TransitiveClosure {
		...
		public TransitiveClosure() {
			all = new DirectedTraversals[DirectedGraph.this.vertices()];
			for(int v = 0; v < DirectedGraph.this.vertices(); v++) {
